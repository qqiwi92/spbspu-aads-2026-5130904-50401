...
stuff::Vector< std::pair< std::string, size_t > > levkin::Edges::getEdges() const
{
...
void levkin::Graph::addEdge(std::string start_vertex, std::string end_vertex, size_t weight)
{
  if (!incoming_.has(end_vertex)) incoming_.add(end_vertex, Edges());
  if (!outgoing_.has(start_vertex)) outgoing_.add(start_vertex, Edges());

  incoming_.at(end_vertex).addEdge(start_vertex, weight);
  outgoing_.at(start_vertex).addEdge(end_vertex, weight);
}

void levkin::Graph::cutEdge(std::string start_vertex, std::string end_vertex, size_t weight)
{
...
stuff::Vector< std::pair< std::string, size_t > > levkin::Graph::getOutbound(std::string vertex) const
{
  ...
  return out_check ? outgoing_.at(vertex).getEdges() : stuff::Vector< std::pair< std::string, size_t > >();
}

stuff::Vector< std::pair< std::string, size_t > > levkin::Graph::getInbound(std::string vertex) const
{
  bool out_check = outgoing_.has(vertex);
  bool in_check = incoming_.has(vertex);
  if (!out_check && !in_check) {
  ...
  return in_check ? incoming_.at(vertex).getEdges() : stuff::Vector< std::pair< std::string, size_t > >();
}


levkin::DB::DB() : graphs_(64, 4) {}

void levkin::DB::addVertex(std::string graph_name, std::string vertex, std::ostream& output)
{
  try {
...
void levkin::DB::addEdge(std::string graph_name, std::string start_vertex, std::string end_vertex, size_t weight)
{
  if (!graphs_.has(graph_name)) {
    graphs_.add(graph_name, Graph(graph_name));
  }
...
{
  try {
    stuff::Vector< std::string > collection = graphs_.at(graph_name).getVertexes();
...
void levkin::DB::showGraphEdges(stuff::Vector< std::pair< std::string, size_t > >& edges, std::ostream& output)
{
...
void levkin::DB::showGraphOutbound(std::string graph_name, std::string vertex, std::ostream& output)
{
  try {
    stuff::Vector< std::pair< std::string, size_t > > paths = graphs_.at(graph_name).getOutbound(vertex);
...
void levkin::DB::showGraphInbound(std::string graph_name, std::string vertex, std::ostream& output)
{
  try {
    stuff::Vector< std::pair< std::string, size_t > > paths = graphs_.at(graph_name).getInbound(vertex);
    showGraphEdges(paths, output);
  } catch (...) {
    output << "<INVALID COMMAND>\n";
  }
...
void levkin::DB::mergeGraphs(
    std::string new_graph, std::string old_graph1, std::string old_graph2, std::ostream& output)
{
...
void levkin::DB::extractGraphs(
    std::string new_graph, std::string old_graph, size_t count_k, stuff::Vector< std::string >& vertexes, std::ostream& output)
{
  ...
  try {
    ...
    for (size_t i = 0; i < boundary; ++i) {
      ...
      stuff::Vector< std::pair< std::string, size_t > > tracks = source_ref.getOutbound(start_pt);
